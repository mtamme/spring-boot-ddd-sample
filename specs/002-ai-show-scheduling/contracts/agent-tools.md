...
These are the tools exposed to the Claude LLM by the AI scheduling agent. Each tool maps directly to an application-layer command or query handler.
...
**Returns**: Confirmation message with the new show ID, or error if scheduling failed (past date, overlap, invalid hall/movie).
