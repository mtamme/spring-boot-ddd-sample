...
**Clock discipline**: The `now` parameter is not persisted — it is a transient construction-time input used solely for
the `Contract.check(scheduledAt.isAfter(now))` invariant. The application-layer command handler obtains it from the
injected `Clock` bean (`clock.instant()`). This satisfies constitution v1.6.0 Clock injection rule while keeping the
domain layer free of `Clock` dependencies.
