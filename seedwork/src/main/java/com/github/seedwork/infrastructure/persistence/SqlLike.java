...
public final class SqlLike {
  ...
  private SqlLike(final String[] keywords) {
    this.keywords = Objects.requireNonNull(keywords);
  }

  public static SqlLike of(final String term) {
    final var keywords = KEYWORDS_SPLIT_PATTERN.split(term);

    return new SqlLike(keywords);
  }
