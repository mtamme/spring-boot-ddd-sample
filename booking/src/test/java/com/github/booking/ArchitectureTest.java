...
class ArchitectureTest {
  ...
  void applicationLayerMustOnlyDependOnApprovedPackages(final JavaClasses classes) {
    ArchRuleDefinition.priority(Priority.HIGH)
      ...
      .should().onlyDependOnClassesThat().resideInAnyPackage(
        ...
        "com.github.seedwork.application..",
        DOMAIN_PACKAGE,
        APPLICATION_PACKAGE)
  ...
  @ArchTest
  void applicationLayerMustOnlyContainUseCaseHandlers(final JavaClasses classes) {
    ArchRuleDefinition.priority(Priority.HIGH)
      .classes()
      .that().resideInAPackage(APPLICATION_PACKAGE)
      .and().areNotNestedClasses()
      .should().beAssignableTo(CommandHandler.class)
      .orShould().beAssignableTo(CommandHandlerWithResult.class)
      .orShould().beAssignableTo(EventHandler.class)
      .orShould().beAssignableTo(QueryHandler.class)
      .because("the application layer must only contain use case handlers")
      .check(classes);
  }

  @ArchTest
  void commandHandlerImplementationsMustFollowConvention(final JavaClasses classes) {
    ArchRuleDefinition.priority(Priority.HIGH)
      .classes()
      .that().areAssignableTo(CommandHandler.class)
      .or().areAssignableTo(CommandHandlerWithResult.class)
      .and().areNotInterfaces()
      .and().areNotNestedClasses()
      .should().resideInAPackage(ROOT_PACKAGE + ".application.*.command..")
      .because("command handler implementations must reside in application.<aggregate>.command")
      .check(classes);
  }

  @ArchTest
  void eventHandlerImplementationsMustFollowConvention(final JavaClasses classes) {
    ArchRuleDefinition.priority(Priority.HIGH)
      .classes()
      .that().areAssignableTo(EventHandler.class)
      .and().areNotInterfaces()
      .and().areNotNestedClasses()
      .should().resideInAPackage(ROOT_PACKAGE + ".application.*.policy..")
      .because("event handler implementations must reside in application.<aggregate>.policy")
      .check(classes);
  }

  @ArchTest
  void queryHandlerImplementationsMustFollowConvention(final JavaClasses classes) {
    ArchRuleDefinition.priority(Priority.HIGH)
      .classes()
      .that().areAssignableTo(QueryHandler.class)
      .and().areNotInterfaces()
      .and().areNotNestedClasses()
      .should().resideInAPackage(ROOT_PACKAGE + ".infrastructure.persistence.*..")
      .because("query handler implementations must reside in infrastructure.persistence.<aggregate>")
      .check(classes);
  }
