...
public interface JpaMessageRepository extends JpaMessageEnqueuer, Repository<Message, Long> {
  ...
  @NativeQuery(name = "Message.lockAllNextDeliverable")
  @Modifying
  int lockAllNextDeliverable(@Param("available_at") Instant availableAt,
                             @Param("lock_id") UUID lockId,
                             @Param("limit") int limit,
                             @Param("max_delivery_count") int maxDeliveryCount,
                             @Param("locked_at") Instant lockedAt);

  @NativeQuery(name = "Message.lockAllNextUndeliverable")
  @Modifying
  int lockAllNextUndeliverable(@Param("available_at") Instant availableAt,
                               @Param("lock_id") UUID lockId,
                               @Param("limit") int limit,
                               @Param("max_delivery_count") int maxDeliveryCount,
                               @Param("locked_at") Instant lockedAt);
