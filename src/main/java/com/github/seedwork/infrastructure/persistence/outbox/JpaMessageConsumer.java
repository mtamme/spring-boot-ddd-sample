...
public class JpaMessageConsumer implements MessageConsumer {
  ...
  @Override
  public List<Message> lockAllDeliverable(final UUID lockId, final int limit) {
    final var lockedAt = clock.instant();
    final var lockCount = repository.lockAllDeliverable(
  ...
  @Override
  public List<Message> lockAllUndeliverable(final UUID lockId, final int limit) {
    final var lockedAt = clock.instant();
    final var lockCount = repository.lockAllUndeliverable(
      lockedAt.plus(lockDuration),
      lockId,
      limit,
      maxDeliveryCount,
  ...
  @Override
  public void requeue(final Long sequenceNumber, final UUID lockId) {
  ...
  @Override
  public void requeueAll(final List<Long> sequenceNumbers, final UUID lockId) {
  ...
  @Override
  public void dequeue(final Long sequenceNumber, final UUID lockId) {
  ...
  @Override
  public void dequeueAll(final List<Long> sequenceNumbers, final UUID lockId) {
    if (sequenceNumbers.isEmpty()) {
      return;
    }
