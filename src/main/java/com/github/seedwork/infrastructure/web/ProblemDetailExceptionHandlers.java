...
public class ProblemDetailExceptionHandlers {
  ...
  private static final Problem DUPLICATE_PROBLEM = new Problem("duplicate");
  ...
  @ExceptionHandler
  public ResponseEntity<ProblemDetail> handleException(final OptimisticLockingFailureException exception) {
  ...
  public ResponseEntity<ProblemDetail> handleException(final DataIntegrityViolationException exception) {
    if (SqlClass.INTEGRITY_CONSTRAINT_VIOLATION.isSubclass(exception.getRootCause())) {
      return toResponseEntity(HttpStatus.CONFLICT, DUPLICATE_PROBLEM.withMessage(exception.getMessage()));
    }
