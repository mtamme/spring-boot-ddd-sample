...
public class Booking extends AggregateRoot {
  ...
  public Booking(final ShowId showId, final BookingId bookingId) {
    ...
    recordEvent(new BookingInitiated(showId(), bookingId()));
  }
  ...
  public void confirm() {
    Contract.check(isInitiated() || isConfirmed(), BookingNotConfirmableException::new);
  ...
  private void markAsConfirmed() {
    ...
    recordEvent(new BookingConfirmed(showId(), bookingId()));
  }

  public void cancel() {
    Contract.check(isInitiated() || isCancelled(), BookingNotCancelableException::new);
  ...
  private void markAsCancelled() {
    ...
    recordEvent(new BookingCancelled(showId(), bookingId()));
  }
