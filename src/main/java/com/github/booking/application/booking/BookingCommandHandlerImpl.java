...
public class BookingCommandHandlerImpl implements BookingCommandHandler {
  ...
  public void reserveSeat(final ReserveSeatCommand command) {
    final var bookingId = new BookingId(command.bookingId());
    final var show = showRepository.findByBookingId(bookingId)
      .orElseThrow(BookingNotFoundException::new);

    show.reserveSeat(bookingId, new SeatNumber(command.seatNumber()));
  ...
  public void releaseSeat(final ReleaseSeatCommand command) {
    ...
    show.releaseSeat(bookingId, new SeatNumber(command.seatNumber()));
    showRepository.save(show);
  }
