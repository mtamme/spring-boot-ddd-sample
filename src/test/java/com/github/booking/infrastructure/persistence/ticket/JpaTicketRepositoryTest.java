...
class JpaTicketRepositoryTest extends PersistenceTest {
  ...
  void findByTicketIdShouldReturnTicket() {
    ...
    // Assert
    transactionTemplate.executeWithoutResult(ts -> {
      final var ticket = ticketRepository.findByTicketId(new TicketId("T0000000000"));

      assertTrue(ticket.isPresent());
      assertEquals(new BookingId("B0000000000"), ticket.get().bookingId());
      assertEquals(new TicketId("T0000000000"), ticket.get().ticketId());
      assertEquals(TicketStatus.ISSUED, ticket.get().status());
      final var showAssignment = ticket.get().showAssignment();
