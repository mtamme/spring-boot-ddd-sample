...
class JpaBookingRepositoryTest extends PersistenceTest {
  ...
  void findByBookingIdShouldReturnBooking() {
    ...
    // Assert
    transactionTemplate.executeWithoutResult(ts -> {
      final var booking = bookingRepository.findByBookingId(new BookingId("B0000000000"));

      assertTrue(booking.isPresent());
      assertEquals(new ShowId("S0000000000"), booking.get().showId());
      assertEquals(new BookingId("B0000000000"), booking.get().bookingId());
