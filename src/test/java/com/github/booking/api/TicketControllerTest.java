...
import com.github.booking.application.ticket.view.ShowAssignmentView;
...
class TicketControllerTest extends ControllerTest {
  ...
  void getTicketShouldReturnGetTicketResponse() {
    ...
    Mockito.when(bookingQueryHandler.getTicket("T0000000000"))
      .thenReturn(new TicketDetailView(
        ...
        ShowAssignmentView.of("TestTitle", "TestName", Instant.EPOCH, "A1")));
    ...
    final var showAssignment = response.getShowAssignment();

    assertEquals("TestTitle", showAssignment.getMovieTitle());
    assertEquals("TestName", showAssignment.getHallName());
    assertEquals(Instant.EPOCH, showAssignment.getScheduledAt());
    assertEquals("A1", showAssignment.getSeatNumber());
  }
  ...
  void listTicketsShouldReturnListTicketsResponse() {
    ...
    assertEquals("B0000000000", booking.getBookingId());
    assertEquals("COMPLETED", booking.getStatus());
    final var showAssignment = ticket.getShowAssignment();
