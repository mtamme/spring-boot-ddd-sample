...
class TicketEventHandlerTest {
  ...
  void onSeatBookedShouldIssueTicket() {
    ...
    Mockito.when(showRepository.findByShowId(new ShowId("S0000000000")))
      .thenReturn(Optional.of(Shows.newShowWithBookedSeats("S0000000000", "B0000000000", "A1")));
    Mockito.when(ticketRepository.nextTicketId())
      .thenReturn(new TicketId("T0000000000"));
    final var ticketCaptor = ArgumentCaptor.forClass(Ticket.class);

    Mockito.doNothing()
      .when(ticketRepository)
      .save(ticketCaptor.capture());
    final var showEventHandler = new TicketEventHandler(ticketRepository, showRepository);
    final var event = new SeatBooked(new ShowId("S0000000000"), new SeatNumber("A1"), new BookingId("B0000000000"));

    // Act
    showEventHandler.onSeatBooked(event);
